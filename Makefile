...
build_win: frontend_build
	GIN_MODE=release & GOOS=windows GOARCH=amd64 go build -ldflags="-s -w -X main.AppMode=prod" -trimpath -o ./bin/win/go-react-local.exe ./main.go

build_linux: frontend_build
	GIN_MODE=release & GOOS=linux GOARCH=amd64 go build -ldflags="-s -w -X main.AppMode=prod" -trimpath -o ./bin/linux/go-react-local ./main.go
